...
@patch("google.genai.Client")
@patch("time.sleep", return_value=None)  # avoid sleeping during tests
def test_gemini_retry_on_503(mock_sleep, mock_client_class):
    """Verifies that transient errors (503) trigger retries."""
    mock_client = MagicMock()
    mock_client_class.return_value = mock_client
    
    # Configure APIError with code 503
    ...
    mock_response = MagicMock()
    mock_response.text = "Hello from Gemini after retry!"
    
    # First call raises 503, second call succeeds
    mock_client.models.generate_content.side_effect = [transient_error, mock_response]
    
    from app.agent.tasks import execute_conversational_task
    res = execute_conversational_task("Hello chatbot")
    
    assert res == "Hello from Gemini after retry!"
    assert mock_client.models.generate_content.call_count == 2

@patch("google.genai.Client")
@patch("time.sleep", return_value=None)
def test_gemini_retry_failures(mock_sleep, mock_client_class):
    """Verifies that the retry helper fails after max attempts (5) for persistent 503."""
    mock_client = MagicMock()
    mock_client_class.return_value = mock_client
    ...
    mock_client.models.generate_content.side_effect = transient_error
    
    from app.agent.tasks import execute_conversational_task
    with pytest.raises(APIError) as exc_info:
        execute_conversational_task("Hello chatbot")
        
    assert exc_info.value.code == 503
    # It should have attempted exactly 5 times
    assert mock_client.models.generate_content.call_count == 5

@patch("google.genai.Client")
@patch("time.sleep", return_value=None)
def test_planner_retry_on_503(mock_sleep, mock_client_class):
    """Verifies that transient errors (503) trigger retries in the planner."""
    mock_client = MagicMock()
    mock_client_class.return_value = mock_client
    ...
    result = plan_task(query="Summarize this", extracted_text="content", file_type="txt")
